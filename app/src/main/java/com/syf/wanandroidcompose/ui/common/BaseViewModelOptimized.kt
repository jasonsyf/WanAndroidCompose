...
abstract class BaseViewModelOptimized<A : Action, S : State> : ViewModel() {
     ...
     * 
     * 使用 Channel.BUFFERED (容量 64) 而不是默认的 RENDEZVOUS (容量 0)
     * 这样可以处理快速连续的用户操作（如快速点击）
     */
     ...
     * UI 状态流
     * 
     * 使用 MutableSharedFlow 而不是 StateFlow，因为有时不需要初始值
     * replay = 1: 新订阅者会收到最新的状态
     * onBufferOverflow = DROP_OLDEST: 缓冲区满时丢弃最旧的状态
     */
    ...
    /**
     * 暴露给外部的状态流
     * distinctUntilChanged: 只在状态真正改变时才发送
     */
    val state: Flow<S> by lazy { _state.distinctUntilChanged() }

    /**
     * 获取当前重放的状态
     * replay 始终为 1，所以最多有一个状态
     */
    val replayState: S?
    ...
    private fun logException(exception: Exception, action: A) {
        if (enableLogging) {
            Timber.tag("MVI-Error").e(exception, "Error handling action: ${action::class.simpleName}")
        }
    }
